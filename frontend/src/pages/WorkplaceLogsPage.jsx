...
function ReviewModal({ log, onClose, onSuccess }) {
  ...
  const handleReview = async () => {
    if (!comment.trim()) {
      toast.error('A comment is required to review.')
      return
    }
    setSubmitting(true)
    try {
      ...
      const msg = err?.detail || err?.message || 'Failed to review log.'
      toast.error(msg)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-slate-800 border border-slate-700/50 rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 space-y-4 overflow-y-auto flex-1">
          ...
          <div>
            <label className="text-xs text-slate-500 uppercase tracking-wider mb-1 block">Your Comment</label>
            <textarea
              value={comment}
      ...
      <div className="relative bg-slate-800 border border-slate-700/50 rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 space-y-4 overflow-y-auto flex-1">
          <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-700/50 space-y-4">
            <div>
              <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Activities</p>
              <p className="text-slate-300 text-sm">{log.activities || 'No activities recorded.'}</p>
            </div>
            {log.challenges && (
              <div>
            ...
            {log.lesson && (
              <div>
                <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Lessons Learned</p>
                <p className="text-slate-300 text-sm">{log.lesson}</p>
              </div>
            )}
          </div>
          ...
          {log.supervisor_comment && (
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4">
              <p className="text-xs text-blue-400 uppercase tracking-wider mb-1">Workplace Supervisor Comment</p>
...
export default function WorkplaceLogsPage() {
  ...
  return (
    <div className="space-y-6">
      {reviewLog && (
        <ReviewModal log={reviewLog} onClose={() => setReviewLog(null)} onSuccess={(id, status, comment) => { handleStatusUpdate(id, status, comment); setReviewLog(null) }} />
      )}
      {detailLog && (
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">Weekly log submissions from your assigned students</p>
      </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700/50">
                  {['Student', 'Company', 'Week', 'Status', 'Submitted', ''].map(h => (
                  ...
                  <tr key={log.id} className="hover:bg-slate-700/20 transition">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-white text-sm font-medium capitalize">{log.student_name}</p>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4 flex gap-2">
                      {log.status === 'submitted' && (
                        <button onClick={() => setReviewLog(log)}
                          className="px-3 py-1.5 bg-blue-600/20 hover:bg-blue-600/40 text-blue-300 border border-blue-500/50 rounded-lg text-xs font-medium transition">
                          Review
                        </button>
                      )}
